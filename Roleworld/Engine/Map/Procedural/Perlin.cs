...
public abstract class Perlin<GradientType>
{
    ...
    public double NoiseTiled(double x, double y = 0.5d, double z = 0.5d, int tileRegion = 2)
    {
        ...
        GradientType V011 = gradients[
            PT[PT[XIndex1] + (cubeZ + 1) % tileRegion] % gradients.Length
        ];
        GradientType V100 = gradients[PT[PT[X1Index] + cubeZ % tileRegion] % gradients.Length];
        GradientType V101 = gradients[
            PT[PT[X1Index] + (cubeZ + 1) % tileRegion] % gradients.Length
        ];
        GradientType V110 = gradients[PT[PT[X1Index1] + cubeZ % tileRegion] % gradients.Length];
        GradientType V111 = gradients[
            PT[PT[X1Index1] + (cubeZ + 1) % tileRegion] % gradients.Length
        ];
        x -= Math.Floor(x);
        y -= Math.Floor(y);
        z -= Math.Floor(z);
...
public class Perlin : Perlin<Perlin.Vector3>
{
    ...
    public Perlin(Func<double, double> smoothingFunction)
        : base(gradients, Dot, smoothingFunction) { }
